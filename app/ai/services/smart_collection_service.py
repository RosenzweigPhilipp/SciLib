...
class SmartCollectionService:
    ...
    def classify_paper(self, title: str, abstract: str = None) -> List[str]:
        """
        Classify a paper into 1-3 research fields.
        ...
        Returns:
            List of 1-3 research field names
        """
        try:
            # Build prompt
            text = f"Title: {title}"
...
Return ONLY a JSON array of 1-3 field names, nothing else. Example: ["Machine Learning", "Computer Vision"]
...
"""
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a research field classification expert. Return only valid JSON arrays."},
            ...
            try:
                ...
                if isinstance(fields, list) and 1 <= len(fields) <= 3:
                    logger.info(f"Classified paper '{title}' into fields: {fields}")
                    return fields
                else:
    ...
    def classify_papers_batch(self, papers: List[Dict[str, Any]]) -> Dict[int, List[str]]:
        """
